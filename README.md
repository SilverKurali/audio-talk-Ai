...
![WebUI 配置](docs/screenshot-webui-history.png)
...
自 v0.1.0 版本起，配置文件中的 API 密钥会以 **AES-256-GCM** 加密后存储为 `enc:<base64>` 形式，解密用的钥匙文件位于 `~/.config/audio-talk-ai/key`（权限 `0600`，仅本人可读；Windows 上为 `%APPDATA%\audio-talk-ai\key`，依赖用户目录 ACL 保护）。明文密钥只在程序运行时存在于内存中。
